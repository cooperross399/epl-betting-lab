...
passed. `Needs fixes`, `Blocked`, or `Missing staging inputs` means the files
are not eligible. The command never copies the files into `data/manual/` or
generates picks.
...
Thursday. Unknown providers are blocked. If you change the policy after
validation, validate again so the receipt records the new policy checksum.
