...
- Historical results could not be refreshed; the model was fitted on the previously stored dataset.
- The upcoming slate could not be refreshed from Football-Data; the provider's staged fixtures were used instead, so the card is about the right fixtures. What is down is the independent source the shadow verifier checks the provider against, which cannot ask its question while the provider is also the answer.
