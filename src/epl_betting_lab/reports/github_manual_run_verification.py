...
TRUSTED_HANDOFF_FIELDS = (
    ...
    "staging_receipt_provider_name",
    "staging_receipt_provider_type",
    "staging_receipt_source_file_path",
    "staging_receipt_source_checksum_sha256",
    "staging_receipt_generated_by",
    "staging_receipt_notes",
    "staging_provider_policy_path",
    "staging_provider_policy_checksum_sha256",
    "staging_provider_policy_match_status",
    "staging_provider_policy_status",
    "staging_provider_allowed",
    "staging_receipt_age_hours",
    "staging_receipt_age_status",
    "staging_provider_policy_timezone",
    "staging_thursday_cutoff_time",
    "staging_thursday_cutoff_at",
    "staging_cutoff_policy_status",
    "current_odds_path",
...
def build_github_manual_run_verification(
    ...
    receipt_provider_name = str(
        handoff.get("staging_receipt_provider_name", "")
    )
    receipt_provider_type = str(
        handoff.get("staging_receipt_provider_type", "unknown")
    )
    receipt_source_path = str(
        handoff.get("staging_receipt_source_file_path", "")
    )
    receipt_source_checksum = str(
        handoff.get("staging_receipt_source_checksum_sha256", "")
    )
    receipt_generated_by = str(
        handoff.get("staging_receipt_generated_by", "")
    )
    receipt_notes = str(handoff.get("staging_receipt_notes", ""))
    provider_policy_path = str(handoff.get("staging_provider_policy_path", ""))
    provider_policy_checksum = str(
        handoff.get("staging_provider_policy_checksum_sha256", "")
    )
    provider_policy_match = str(
        handoff.get("staging_provider_policy_match_status", "Not checked")
    )
    provider_policy_status = str(
        handoff.get("staging_provider_policy_status", "Not checked")
    )
    provider_allowed = handoff.get("staging_provider_allowed")
    receipt_age_hours = handoff.get("staging_receipt_age_hours")
    receipt_age_status = str(
        handoff.get("staging_receipt_age_status", "Not checked")
    )
    provider_policy_timezone = str(
        handoff.get("staging_provider_policy_timezone", "")
    )
    thursday_cutoff_time = str(
        handoff.get("staging_thursday_cutoff_time", "")
    )
    thursday_cutoff_at = str(
        handoff.get("staging_thursday_cutoff_at", "")
    )
    cutoff_policy_status = str(
        handoff.get("staging_cutoff_policy_status", "Not checked")
    )
    ...
    proof_values = (
        ...
        ("Staging provider", receipt_provider_name),
        ("Staging provider type", receipt_provider_type),
        ("Staging source file", receipt_source_path or "Not provided"),
        (
            "Staging source checksum",
            receipt_source_checksum or "Not available",
        ),
        ("Staging receipt generated by", receipt_generated_by or "Not provided"),
        ("Provider policy", provider_policy_status),
        ("Receipt age status", receipt_age_status),
        ("Thursday cutoff status", cutoff_policy_status),
        ("Odds path", odds_path),
                ...
                ready_requirements.update(
                    {
                        ...
                        == "Verified",
                        "Staging provider policy path": bool(provider_policy_path),
                        "Staging provider policy checksum": bool(
                            SHA256_PATTERN.fullmatch(
                                provider_policy_checksum.lower()
                            )
                        ),
                        "Staging provider policy snapshot": provider_policy_match
                        == "Verified",
                        "Staging provider policy status": provider_policy_status
                        == "Provider allowed",
                        "Staging provider allowed": provider_allowed is True,
    ...
    summary = {
        ...
        "staging_receipt_provider_name": receipt_provider_name,
        "staging_receipt_provider_type": receipt_provider_type,
        "staging_receipt_source_file_path": receipt_source_path,
        "staging_receipt_source_checksum_sha256": receipt_source_checksum,
        "staging_receipt_generated_by": receipt_generated_by,
        "staging_receipt_notes": receipt_notes,
        "staging_provider_policy_path": provider_policy_path,
        "staging_provider_policy_checksum_sha256": provider_policy_checksum,
        "staging_provider_policy_match_status": provider_policy_match,
        "staging_provider_policy_status": provider_policy_status,
        "staging_provider_allowed": provider_allowed,
        "staging_receipt_age_hours": receipt_age_hours,
        "staging_receipt_age_status": receipt_age_status,
        "staging_provider_policy_timezone": provider_policy_timezone,
        "staging_thursday_cutoff_time": thursday_cutoff_time,
        "staging_thursday_cutoff_at": thursday_cutoff_at,
        "staging_cutoff_policy_status": cutoff_policy_status,
...
def render_github_manual_run_verification(
    ...
    lines = [
        ...
        (
            "- Provider: "
            f"**{summary['staging_receipt_provider_name'] or 'unknown'}** "
            f"({summary['staging_receipt_provider_type']})"
        ),
        (
            "- Source file: "
            f"`{summary['staging_receipt_source_file_path'] or 'not provided'}`"
        ),
        (
            "- Source SHA-256: "
            f"`{summary['staging_receipt_source_checksum_sha256'] or 'not available'}`"
        ),
        (
            "- Generated by: "
            f"{summary['staging_receipt_generated_by'] or 'not provided'}"
        ),
        f"- Provider policy: **{summary['staging_provider_policy_status']}**",
        (
            "- Receipt age: "
            f"**{summary['staging_receipt_age_status']}** "
            f"({summary['staging_receipt_age_hours']} hour(s))"
        ),
        (
            "- Thursday cutoff: "
            f"**{summary['staging_cutoff_policy_status']}** "
            f"({summary['staging_thursday_cutoff_at'] or 'not available'})"
        ),
