...
def test_provider_shadow_comparison_dashboard_action_is_report_only(
    tmp_path, monkeypatch
) -> None:
    output_dir = tmp_path / "outputs"
...
def test_provider_human_acceptance_verification_dashboard_action_is_report_only(
    tmp_path, monkeypatch
) -> None:
    output_dir = tmp_path / "outputs"
    receipt_path = output_dir / "selected_receipt.json"
    expected = {"verdict": "Verified for allowlist PR review"}

    def fake_save(provider_name, selected_output_dir, *, receipt_path=None):
        assert provider_name == "odds_api"
        assert selected_output_dir == output_dir
        assert receipt_path == output_dir / "selected_receipt.json"
        return expected
    ...
    assert run_provider_human_acceptance_receipt_verification(
        "odds_api",
        output_dir,
        receipt_path,
    ) == expected
...
def test_provider_allowlist_evidence_bundle_dashboard_action_is_report_only(
    tmp_path, monkeypatch
) -> None:
    output_dir = tmp_path / "outputs"
    policy_path = tmp_path / "policy.json"
    expected = {"verdict": "Evidence bundle ready for PR review"}
    ...
    assert run_provider_allowlist_evidence_bundle(
        "odds_api",
        output_dir,
        policy_path,
    ) == expected
...
def test_provider_allowlist_evidence_bundle_verification_dashboard_action_is_read_only(
    tmp_path, monkeypatch
) -> None:
    output_dir = tmp_path / "outputs"
    bundle_path = output_dir / "selected_bundle.json"
    expected = {"verdict": "Evidence bundle verified for PR approval review"}
    ...
    def fake_save(
        ...
        *,
        ...
        assert provider_name == "odds_api"
        assert selected_output_dir == output_dir
        assert bundle_path == output_dir / "selected_bundle.json"
        return expected
    ...
    assert run_provider_allowlist_evidence_bundle_verification(
        "odds_api",
        output_dir,
        bundle_path,
    ) == expected
