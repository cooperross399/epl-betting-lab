...
def _probs(n=400, seed=1):
    ...
    for col, prob in (("AvgH", p[:, 0]), ("AvgD", p[:, 1]), ("AvgA", p[:, 2])):
        frame[col] = 1 / np.clip(prob * 1.05, 0.03, 0.97)          # 5% overround
        frame["AvgC" + col[-1]] = frame[col] * rng.uniform(0.97, 1.03, n)
    frame["AvgO"] = 1 / (frame["p_over"] * 1.04); frame["AvgU"] = 1 / ((1 - frame["p_over"]) * 1.04)
...
def test_evaluation_is_sorted_by_train_clv_and_carries_test_columns():
    table = evaluate_out_of_sample(_probs(), "1x2", train_seasons=(2324,), test_seasons=(2526,),
...
def test_totals_market_is_scored_too():
    table = evaluate_out_of_sample(_probs(), "total_2_5", train_seasons=(2324,), test_seasons=(2526,),
